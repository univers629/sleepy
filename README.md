...
> nano config.json
> ```
- 会出现按照下面的说明编辑`config.json`文件，编辑完成后按`Ctrl+X`后按`Y`确认修改，最后按`回车键`退出(启动前记得可以修改自己的网站图标/static/favicon.ico)
> ```python
> {
> "version": "2025.1.18.1",
> "debug": false,
...
> [!IMPORTANT]
> 
> docker服务不需要再写systemd配置保活，已在docker-compose.yml中声明了持续运行
